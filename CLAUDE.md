...
Versions are kept **side-by-side**, not overwritten. `Booking engine v5.html` (~4446 lines, ~2.6 MB) is the current canonical app. Older files (`Booking engine v4.html`, `v3.2`, `v3.3 (offline)`, `V2`, `v3-bookings`, `Booking sheets unified V1`) are historical and should generally not be edited unless explicitly requested.
...
`saveAll()` in v4 overwrote each .xlsx wholesale, so concurrent saves on the SMB share clobbered each other. v5 adds a `/* === Sync (multi-user) === */` banner (between `Save / reload` and `File source`, around line 1966) that makes saves merge against the current disk version. `'fs'` mode only — `'fp'` is sync-disabled by design. v4 retains the unsafe single-user behaviour.

- **Op-log replay.** `writeBooking` (~1475), `setBookingMeta` (~1748), `setSheetMeta` (~1775) each push a structured op into `f.pendingOps` next to their existing `f.dirty = true`. `bookingTempId` threads a create through later meta ops on the same row (cancel-after-create); `txnId` keeps multi-op flows like `postponeBooking` atomic at replay. A module-level `replaying` flag suppresses `recordOp` and the `rebuildBookings()` call inside `writeBooking`.
...
- **`reloadFile(fileEntry)`** — extracted from `loadFiles`'s per-file body, reused by poller and conflict path; does not touch view state.
...
- **Crash recovery.** Each `recordOp` mirrors the file's full op queue under `localStorage['reefdesk:pendingOps:<dirHandle.name>:<filename>']`; the splice-after-save clears it. `openDirectory` calls `offerCrashRecovery()` after `loadFiles` to surface a Restore/Discard modal when a previous session left ops behind. `'fp'` mode skips persistence (no stable folder identity).
- **Save-in-flight UI.** A `#saving-indicator` "Saving — please don't close" pill in the header is toggled by `updateSavingIndicator()` on every `STATE.saving.add/delete`. The `beforeunload` handler also blocks on `STATE.saving.size > 0`, not just dirty files.
